...
$("#serverRequestForm").onsubmit=async e=>{e.preventDefault();try{await api("/api/profile/server-requests",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({serverName:$("#requestServerName").value,websiteUrl:$("#requestWebsite").value,cap:Number($("#requestCap").value),rates:$("#requestRates").value,serverType:$("#requestType").value,openedAt:$("#requestOpenedAt").value,description:$("#requestDescription").value})});show("Sunucu isteğiniz yöneticiye gönderildi.","good");e.target.reset();$("#requestCap").value=110;$("#requestRates").value="1x";await init()}catch(e){show(e.message,"bad")}};
